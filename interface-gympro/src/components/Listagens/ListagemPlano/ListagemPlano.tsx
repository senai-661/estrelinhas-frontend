...
function ListagemPlanos(): JSX.Element {
    ...
    const planosFiltrados = planos.filter((p) =>
        `${p.codPlano} ${p.tipoPlano} ${p.statusPlano}`
            .toLowerCase()
                    ...
                    <tbody>
                        {currentPlanos.length > 0 ? (
                            currentPlanos.map((plano, index) => (
                                <tr
                                    key={plano.codPlano ?? index}
                                    onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#fff8f5')}
                                    onMouseLeave={e => (e.currentTarget.style.backgroundColor = '#fff')}
                                >
                                    <td style={tdStyle}>{plano.codPlano ?? '—'}</td>
                                    <td style={{ ...tdStyle, fontWeight: 500 }}>{plano.tipoPlano ?? '—'}</td>
                                    <td style={tdStyle}>{plano.duracaoDias ?? '—'} dias</td>
                                    <td style={{ ...tdStyle, fontWeight: 700 }}>
                                        {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(plano.valor)}
                                    </td>
                                    <td style={tdStyle}>
                                        <span style={{
                                            backgroundColor: plano.statusPlano === 'ativo' ? '#dcfce7' : '#fee2e2',
                                            color: plano.statusPlano === 'ativo' ? '#16a34a' : '#ef4444',
                                            padding: '3px 12px',
                                            borderRadius: '999px',
                                            fontSize: '0.78rem',
                                            fontWeight: 600,
                                    ...
                                    <td style={{ ...tdStyle, textAlign: 'center' }}>
                                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                                            <button
                                                onClick={() => navigate(`/detalhes/plano/${plano.codPlano}`)}
