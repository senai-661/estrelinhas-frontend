...
function ListagemMatriculas(): JSX.Element {
    ...
    const matriculasFiltradas = matriculas.filter((m) =>
        `${m.idMatricula} ${m.statusMatricula} ${m.formaPagamento}`
            .toLowerCase()
                        ...
                        {currentMatriculas.length > 0 ? (
                            currentMatriculas.map((matricula, index) => (
                                ...
                                >
                                    ...
                                    <td style={tdStyle}>
                                        {formatarData(matricula.dataMatricula)} → {formatarData(matricula.dataVencimento)}
                                    </td>
                                    <td style={{ ...tdStyle, fontWeight: 700 }}>
                                        {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(matricula.valorPago))}
                                    </td>
                                    <td style={tdStyle}>{matricula.formaPagamento ?? '—'}</td>
                                    <td style={tdStyle}>
                                        <span style={{
                                            backgroundColor: matricula.statusMatricula === 'ATIVA' ? '#dcfce7' : '#fee2e2',
                                            color: matricula.statusMatricula === 'ATIVA' ? '#16a34a' : '#ef4444',
                                            padding: '3px 12px',
                                            borderRadius: '999px',
                                            fontSize: '0.78rem',
                                            fontWeight: 600,
